...
{
    public partial class functional_test : Form
    {
        ...
        {
            Process[] processes = Process.GetProcessesByName("Aurora Main Board Test");
            foreach (var process1 in processes)
            {
                process1.Kill();
            }
            Thread.Sleep(200);
            string str = @"C:\Users\Administrator\Desktop\1922\Centum Package\Programs\Digi Ethernet\DeviceDiscovery.exe";
            Process process = new Process();
            process.StartInfo.FileName = str;
            Thread.Sleep(500);
            process.Start();
            Thread.Sleep(500);
            ethernet_configration P = new ethernet_configration();
            this.Hide();
            P.ShowDialog();
        }
        ...
        private void functional_test_Load(object sender, EventArgs e)
        {
